...
export default function PsychometricAdmin() {
  ...
  const generatePDFReport = async (attemptId: number) => {
    try {
      ...
      pdf.setFontSize(12);
      pdf.setTextColor(100, 100, 100);
      pdf.text('Q361 Business Management System by Qanzak Global', 105, 220, { align: 'center' });
      ...
      pdf.text(`Report generated by Q361 Business Management System | Page ${currentPage} of ${currentPage}`, 105, 280, { align: 'center' });
                        ...
                        {attempt.status === 'completed' && (
                          <>
                            <Button 
                              ...
                              size="sm"
                              onClick={() => {
                                window.location.href = `/psychometric-report/${encodeURIComponent(attempt.candidateEmail)}`;
                              }}
