...
import { Plus, Save, Copy, User, Phone, Mail, MessageCircle, Clock, Trash2, AlertTriangle, RefreshCw, TrendingUp, Calendar, History, ChevronRight } from 'lucide-react';
...
export default function CrmDailyLog() {
  const { user } = useAuth();
  ...
  // Fetch recent daily logs history (last 30 days)
  const { data: recentLogs = [] } = useQuery({
    queryKey: ['/api/crm/daily-meetings/history'],
    queryFn: async () => {
      const thirtyDaysAgo = new Date();
      thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
      const response = await fetch(`/api/crm/daily-meetings?from=${thirtyDaysAgo.toISOString().split('T')[0]}`);
      if (!response.ok) return [];
      const data = await response.json();
      // Sort by date descending (most recent first)
      return data.sort((a: any, b: any) => new Date(b.meetingDate).getTime() - new Date(a.meetingDate).getTime());
    },
  });
  ...
  // Load existing record data when todayRecord is available (for viewing/editing past logs)
  useEffect(() => {
    if (todayRecord) {
      setFormData({
        totalSessions: todayRecord.totalSessions || 0,
        inductionSessions: todayRecord.inductionSessions || 0,
        followUpSessions: todayRecord.followUpSessions || 0,
        cancelledRescheduled: todayRecord.cancelledRescheduled || 0,
        cancelReason: todayRecord.cancelReason || '',
        totalInquiries: todayRecord.totalInquiries || 0,
        newLeads: todayRecord.newLeads || 0,
        followUps: todayRecord.followUps || 0,
        closedDeals: todayRecord.closedDeals || 0,
        callsPlaced: todayRecord.callsPlaced || 0,
        callsReceived: todayRecord.callsReceived || 0,
        emailsSent: todayRecord.emailsSent || 0,
        whatsappMessages: todayRecord.whatsappMessages || 0,
        preCounseling: todayRecord.preCounseling || 0,
        preCounselingNotes: todayRecord.preCounselingNotes || '',
        totalCallDuration: todayRecord.totalCallDuration || 0,
        averageCallDuration: todayRecord.averageCallDuration || 0,
        notes: todayRecord.notes || '',
        summary: todayRecord.summary || '',
        sessionNotes: todayRecord.sessionNotes || '',
        communicationNotes: todayRecord.communicationNotes || '',
        timeMetricsNotes: todayRecord.timeMetricsNotes || '',
        criticalClientsNotes: todayRecord.criticalClientsNotes || '',
        ceoClientsNotes: todayRecord.ceoClientsNotes || '',
        googleReviewsNotes: todayRecord.googleReviewsNotes || '',
        criticalClients: todayRecord.criticalClients || [],
        ceoClients: todayRecord.ceoClients || [],
        ceoAvailability: todayRecord.ceoAvailability || { status: '', availableHours: '', notes: '' },
        googleReviews: todayRecord.googleReviews || [],
      });
      // Also load CEO availability fields
      if (todayRecord.ceoAvailability) {
        setCeoAvailStatus(todayRecord.ceoAvailability.status || '');
        setCeoAvailHours(todayRecord.ceoAvailability.availableHours || '');
        setCeoAvailNotes(todayRecord.ceoAvailability.notes || '');
        if (todayRecord.ceoAvailability.schedule) {
          setCeoSchedule(todayRecord.ceoAvailability.schedule);
        }
      }
    } else {
      // Reset form when no record exists for selected date
      setFormData({
        totalSessions: 0,
        inductionSessions: 0,
        followUpSessions: 0,
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
            <Card>
              <CardHeader className={`text-white ${todayRecord ? 'bg-gradient-to-r from-amber-500 to-amber-600' : 'bg-gradient-to-r from-blue-500 to-blue-600'}`}>
                <CardTitle className="flex items-center gap-2">
                  {todayRecord ? (
                    <>
                      <History size={20} /> Viewing Daily Log - {format(new Date(date), 'MMMM dd, yyyy')}
                      <Badge className="bg-white/20 text-white ml-2">Editing Existing</Badge>
                    </>
                  ) : (
                    <>
                      <Plus size={20} /> New Daily Activity Log - {format(new Date(date), 'MMMM dd, yyyy')}
                      <Badge className="bg-white/20 text-white ml-2">New Entry</Badge>
                    </>
                  )}
                </CardTitle>
              </CardHeader>
          ...
          <div className="space-y-4">
            ...
            <Card>
              <CardHeader className="bg-gradient-to-r from-purple-500 to-purple-600 text-white">
                <CardTitle className="flex items-center gap-2">
                  <History size={18} /> Recent Daily Logs
                </CardTitle>
              </CardHeader>
              <CardContent className="p-0">
                {recentLogs.length > 0 ? (
                  <div className="max-h-80 overflow-y-auto divide-y divide-gray-100">
                    {recentLogs.slice(0, 10).map((log: any) => {
                      const logDate = new Date(log.meetingDate);
                      const isSelected = format(logDate, 'yyyy-MM-dd') === date;
                      const isToday = format(logDate, 'yyyy-MM-dd') === format(new Date(), 'yyyy-MM-dd');
                      
                      return (
                        <button
                          key={log.id}
                          onClick={() => setDate(format(logDate, 'yyyy-MM-dd'))}
                          ...
                          data-testid={`log-history-item-${log.id}`}
                        >
                          <div className="flex-1">
                            ...
                            <div className="flex gap-3 mt-1 text-xs text-gray-600">
                              <span>📞 {log.totalSessions || 0} sessions</span>
                              <span>📋 {log.totalInquiries || 0} inquiries</span>
                              <span>✅ {log.closedDeals || 0} deals</span>
                            </div>
                          </div>
                  ...
                  <div className="p-6 text-center text-gray-500">
                    ...
                    <p className="text-xs text-gray-400 mt-1">Save your first daily log to see it here</p>
                  </div>
                )}
              </CardContent>
            </Card>
            ...
            {todayRecord && format(new Date(date), 'yyyy-MM-dd') !== format(new Date(), 'yyyy-MM-dd') && (
              <Button
                ...
                data-testid="btn-create-new-log"
              >
                <Plus className="mr-2" size={18} />
                Create New Daily Log (Today)
              </Button>
            )}
          </div>
        </div>
